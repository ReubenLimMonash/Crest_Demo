    ...
    with open(csv_file, 'w+', buffering=1) as f:
        ...
        while True:
            try:
                ...
                seq = remainder[0]
                txTime = remainder[1].split('u')[0]
                delay = rxTime - float(txTime)

                delay = ping(UAV_IP)
                print("Average rtt: {}".format(delay))
                # print("Received a message {} with seq {} at time {} with delay {}".format(data.decode('utf-8'), seq, rxTime, delay))
...
def gcs_send_app(send_soc, uav_ip, uav_port, send_int, pkt_size, csv_file):
    ...
    with open(csv_file, 'w+', buffering=1) as f:
        ...
        try:
            while(True):
                send_time = time.time()
                send_data = str(i) + '-' + str(send_time) + 'g' * (pkt_size-len(str(i)+str(send_time)))
